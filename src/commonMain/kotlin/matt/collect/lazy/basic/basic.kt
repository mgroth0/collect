...
open class BasicLazyList<E>(
  lazyValues: List<LazyValue<E>>, startIndex: Int = 0, endIndex: Int? = null
): LazyList<E> {

  protected open val lazyValues = lazyValues.subList(startIndex, endIndex ?: (lazyValues.size))

  override val size get() = lazyValues.size

  override fun get(index: Int) = lazyValues[index].value

  override fun isEmpty() = lazyValues.isEmpty()
  ...
  override fun subList(fromIndex: Int, toIndex: Int) = BasicLazyList(lazyValues, fromIndex, toIndex)
  ...
  override fun contains(element: E) = indexOf(element) >= 0
}